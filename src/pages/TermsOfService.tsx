...
export const TermsOfService = () => {
  ...
  useEffect(() => {
    const fetchTerms = async () => {
      try {
        ...
        console.error(err);
        setError('Terms of Service is currently unavailable.');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return (
    <div className="bg-slate-50 min-h-screen pb-20 font-sans">
      <SubHeaderBar
        breadcrumbs={[{ label: 'Home', href: '/' }, { label: data?.page_title || 'Terms of Service' }]}
        title={data?.page_title || 'TERMS OF SERVICE'}
        showSearch={false}
      />
              ...
              <div className="space-y-8">
                {data.sections && data.sections.map((section, idx) => (
                  <div key={idx} className="prose prose-slate max-w-none">
                    ...
                    <div 
                      className="text-slate-600 leading-relaxed text-sm md:text-base prose-p:mb-4 prose-ul:list-disc prose-ul:ml-6 prose-ul:mb-4 prose-li:mb-2 prose-a:text-fitis-blue hover:prose-a:text-blue-700" 
